...
<section class="min-h-screen pt-32 pb-20 flex items-center justify-center bg-slate-50">
    <div class="w-full max-w-md bg-white p-10 rounded-3xl shadow-xl border border-slate-100 animate-slide-up">
        <h2 class="text-3xl font-black text-black mb-8 text-center uppercase tracking-tighter">Login</h2>

        <?php if(!empty($_SESSION['flash_success'])): ?>
            <div class="bg-lime-100 text-lime-800 p-4 rounded-xl mb-6 text-sm font-bold text-center">
        ...
        <form action="/login" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="password" name="password" required class="w-full px-5 py-4 rounded-2xl border-2 border-slate-200 bg-white focus:ring-4 focus:ring-accent/20 focus:border-black transition-all outline-none font-medium">
            </div>
        ...
        <p class="mt-8 text-center text-slate-500 font-medium text-sm">
            Não tem uma conta? <a href="/register" class="text-black font-bold hover:underline">Cadastre-se</a>
        </p>
    </div>
</section>
